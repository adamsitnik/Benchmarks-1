...
{
    public sealed class RawSocketConnectionListener : IConnectionListener
    {
        private readonly SocketTransportOptions _options;
        
        private Socket _listenSocket;

        public RawSocketConnectionListener(EndPoint endpoint, SocketTransportOptions options)
        {
            _options = options;
            EndPoint = endpoint;
        }
        ...
        public async ValueTask<ConnectionContext> AcceptAsync(CancellationToken cancellationToken = new CancellationToken())
        {
            while (true)
            {
                try
                {
                    ...
                    return new RawSocketConnection(acceptSocket);
                }
